			...
			switch (msg.what){
				case 1:
					...
					if(success){
						...
						Intent i = new Intent(AddActivity.this,ReciteListActivity.class);
						startActivity(i);
						}else{
...
//        chPreTreat = (CheckBox) findViewById(R.id.ch_pretreat);
		...
		finish.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(pname.equals(""))
				{
					...
					saveParagraph();
				    Intent intent = new Intent(AddActivity.this,ReciteListActivity.class);
	...
	private void saveParagraph() {
		...
		String name = para_name.getText().toString();
		String content = tvResult.getText().toString();
		if (_id == -1){ // 添加
			Paragraph p = new Paragraph(name, content, 1);
			dao.insert(p);
		} else {        // 编辑后更新
			Paragraph p = new Paragraph(_id, name, content, 1);
			dao.update(p);
		}
	}
...
//                    }
	...
	private void sendTo(final String account, final String pname,final String cont){
		new Thread(new Runnable(){
    		public void run(){
    			...
    			try{
    				...
    				out.writeBytes("account="+account+"&pname="+pname+"&content="+cont);
